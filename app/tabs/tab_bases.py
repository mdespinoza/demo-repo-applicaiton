...
def layout():
    ...
    return html.Div([
        ...
        dbc.Row([
            dbc.Col([
                ...
                dcc.Dropdown(
                    ...
                    multi=True, placeholder="All Components",
                ),
            ], md=3),
            dbc.Col([
        ...
        dbc.Row([
            dbc.Col([
                chart_container("bases-map", "Military Installations Map", height=500, info="Interactive map showing locations of US military installations, color-coded by branch. Hover over markers for base details."),
            ], md=6),
            dbc.Col(chart_container("bases-density", "Base Concentration by State", height=500, info="Choropleth showing the number of military installations per state with individual base locations overlaid as white dots"), md=6),
        ]),

        # --- High-Level Summaries ---
        # Status overview + Bases by branch
        dbc.Row([
            dbc.Col(chart_container("bases-status-donut", "Operational Status Overview", info="Donut chart showing the proportion of bases by operational status (Active, Closed, Surplus, etc.)"), md=6),
            dbc.Col(chart_container("bases-by-branch", "Bases by Branch", info="Stacked bar chart of base counts by military branch, showing operational status breakdown"), md=6),
        ]),

        # --- State-Level Detail ---
        # Bases by state + Joint base analysis
        dbc.Row([
            dbc.Col(chart_container("bases-by-state", "Bases by State (Top 20)", info="Stacked bar chart showing states with the most military bases, broken down by branch"), md=6),
            dbc.Col(chart_container("bases-joint-analysis", "Joint Base Analysis", info="Grouped bar chart showing which military branches participate in joint base operations and how many bases are joint vs non-joint"), md=6),
        ]),

        # --- Physical Characteristics ---
        # Base size + Area vs perimeter
        dbc.Row([
            dbc.Col(chart_container("bases-size-box", "Base Size Distribution by Branch", info="Box plot showing the distribution of base area (AREA column) for each military branch. Outliers indicate exceptionally large installations."), md=6),
            dbc.Col(chart_container("bases-area-perimeter", "Area vs Perimeter Scatter", info="Scatter plot comparing base area and perimeter by branch. Outliers indicate unusually shaped installations."), md=6),
        ]),

        # --- Drill-Down ---
        # Sunburst + small multiples
        dbc.Row([
            dbc.Col(chart_container("bases-sunburst", "State / Component / Base Hierarchy", info="Sunburst chart showing the hierarchy of states, branches, and individual bases. Click to drill down. Top 15 states shown."), md=6),
            dbc.Col(chart_container("bases-small-multiples", "Geographic Distribution by Branch", info="Small multiple maps showing base locations for each military branch separately"), md=6),
        ]),
...
def update_bases(components, statuses, states, joints):
    ...
    for comp in sorted(df_map["COMPONENT"].unique()):
        ...
        map_fig.add_trace(go.Scattergeo(
            ...
            hovertext=comp_df.apply(
                lambda r: f"<b>{r['Site Name']}</b><br>{r['COMPONENT']}<br>{r['State Terr']}<br>{r['Oper Stat']}", axis=1
            ),
            hoverinfo="text",
        ))
    ...
    state_fig = px.bar(
        state_comp, x="Count", y="State Terr", color="COMPONENT",
    ...
    branch_fig = px.bar(
        branch_status, x="COMPONENT", y="Count", color="Oper Stat",
        template=PLOTLY_TEMPLATE,
    )
    ...
    sunburst_fig = px.sunburst(
        sun_df, path=["State Terr", "COMPONENT", "Site Name"], values="count",
        template=PLOTLY_TEMPLATE,
    )
    ...
    for i, branch in enumerate(branches[:n_branches]):
        ...
        sm_fig.add_trace(
            go.Scattergeo(
                lat=branch_df["lat"], lon=branch_df["lon"],
                mode="markers",
    ...
    for i in range(1, n_branches + 1):
        sm_fig.update_geos(
            scope="usa", showland=True, landcolor="#1A2332",
    ...
    density_fig.add_trace(go.Choropleth(
    ...
    density_fig.add_trace(go.Scattergeo(
        lat=df["lat"], lon=df["lon"],
        mode="markers",
        marker=dict(size=3, color="white", opacity=0.4),
        hovertext=df["Site Name"], hoverinfo="text",
        showlegend=False,
    ))
    ...
    if len(joint_comp) > 0:
        joint_fig = px.bar(
            joint_comp, x="COMPONENT", y="Count", color="Joint Base",
    ...
    if len(ap_df) > 0:
        ap_fig = px.scatter(
            ap_df, x="AREA", y="PERIMETER",
            color="COMPONENT", hover_name="Site Name",
            template=PLOTLY_TEMPLATE,
    ...
    return (map_fig, state_fig, branch_fig, sunburst_fig, sm_fig,
            density_fig, size_box_fig, joint_fig, ap_fig, status_fig)
