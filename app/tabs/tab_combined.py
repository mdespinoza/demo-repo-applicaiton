...
def _build_combined_data():
    ...
    equip_by_state = equip_df.groupby("State").agg(
    ...
    bases_by_state = bases_df.groupby("state_abbrev").agg(
...
def layout():
    return html.Div([
        ...
        # --- Overview ---
        dbc.Row([
            dbc.Col(chart_container("combined-temporal", "Temporal Equipment Overlay", height=500, info="Bar chart of yearly equipment acquisition value with a reference line showing total base count for context"), md=6),
            dbc.Col(chart_container("combined-dual-map", "Equipment Value + Base Locations", height=500, info="Dual-layer map: blue choropleth shows equipment acquisition value by state, red dots mark military base locations"), md=6),
        ]),

        # --- State-Level Comparisons ---
        # Scatter + Quadrant (both plot bases vs equipment as scatter)
        dbc.Row([
            dbc.Col(chart_container("combined-scatter", "Base Count vs Equipment Value by State", info="Scatter plot showing correlation between number of bases and equipment value per state. Bubble size reflects equipment count. OLS trendline fitted."), md=6),
            dbc.Col(chart_container("combined-quadrant", "Quadrant Analysis", info="Scatter plot with median dividers creating four quadrants: high/low base count vs high/low equipment value for strategic assessment"), md=6),
        ]),

        # Butterfly + Equip-per-Base (both are state-level horizontal bar rankings)
        dbc.Row([
            dbc.Col(chart_container("combined-butterfly", "Bases vs Equipment Value (Top 20 States)", info="Butterfly chart comparing normalized base counts (left) and equipment values (right) for the top 20 states"), md=6),
            dbc.Col(chart_container("combined-equip-per-base", "Equipment-per-Base Ratio (Top 25)", info="Horizontal bar chart showing equipment value divided by number of bases per state — identifies states with high equipment relative to military presence"), md=6),
        ]),

        # --- Aggregate Summaries ---
        # Regional Radar + Combined Ranking
        dbc.Row([
            dbc.Col(chart_container("combined-radar", "Regional Comparison Radar", info="Radar chart comparing regions across four dimensions: equipment value, equipment count, base count, and number of agencies (all normalized 0-100)"), md=6),
            dbc.Col(chart_container("combined-ranking", "Combined State Ranking", info="Composite score ranking of top 25 states (50% base count + 50% equipment value), colored by census region"), md=6),
        ]),

        # --- Multi-Dimensional Cross-Cuts ---
        # Heatmap + Agency Bubble
        dbc.Row([
            dbc.Col(chart_container("combined-heatmap", "Branch vs Equipment Category Heatmap", info="Heatmap of equipment values (log10 scale) across military branches and top 10 equipment categories"), md=6),
            dbc.Col(chart_container("combined-bubble", "Agency Reach Bubble Chart", info="Bubble chart showing the three-way relationship: agencies served (x), base count (y), and equipment value (bubble size)"), md=6),
        ]),
...
def update_combined(regions, min_bases):
    ...
    dual_fig.add_trace(go.Choropleth(
    ...
    dual_fig.add_trace(go.Scattergeo(
    ...
    scatter_fig = px.scatter(
        combined_filtered, x="base_count", y="equipment_value",
    ...
    butterfly_fig.add_trace(go.Bar(
        y=top20["state_name"],
    ...
    butterfly_fig.update_layout(
        barmode="relative", template=PLOTLY_TEMPLATE,
    ...
    if len(combined_for_rank) > 0:
        ...
        combined_for_rank["composite_score"] = 0.5 * combined_for_rank["norm_bases"] + 0.5 * combined_for_rank["norm_value"]
        combined_for_rank = combined_for_rank.nlargest(25, "composite_score").sort_values("composite_score", ascending=True)

        ranking_fig = px.bar(
            combined_for_rank, x="composite_score", y="state_name",
    ...
    if len(yearly_val) > 0:
        temporal_fig.add_trace(
            go.Bar(x=yearly_val["Year"], y=yearly_val["Value"], name="Acquisition Value",
                   marker_color=COLORS["secondary"], opacity=0.8),
            secondary_y=False,
        )
        temporal_fig.add_trace(
            go.Scatter(x=[yearly_val["Year"].min(), yearly_val["Year"].max()],
                       y=[total_bases, total_bases],
                       mode="lines", name=f"Total Bases ({int(total_bases)})",
    ...
    region_agg = combined.groupby("region").agg(
    ...
    radar_fig.update_layout(
        ...
        legend=dict(orientation="h", yanchor="bottom", y=1.02, xanchor="left", x=0),
        margin=dict(l=40, r=40, t=30, b=20), height=450,
    )
    ...
    if len(epb) > 0:
        epb_fig = px.bar(
            epb, x="equip_per_base", y="state_name", orientation="h",
    ...
    if len(quad_df) > 0:
        ...
        quadrant_fig = px.scatter(
            quad_df, x="base_count", y="equipment_value",
            color="region", color_discrete_map=REGION_COLORS,
            hover_name="state_name", template=PLOTLY_TEMPLATE,
            labels={"base_count": "Number of Bases", "equipment_value": "Equipment Value ($)"},
        )
        # Add median lines
        quadrant_fig.add_hline(y=med_value, line_dash="dash", line_color="rgba(255,255,255,0.3)",
                               annotation_text="Median Value", annotation_position="top left")
        quadrant_fig.add_vline(x=med_bases, line_dash="dash", line_color="rgba(255,255,255,0.3)",
                               annotation_text="Median Bases", annotation_position="top right")
    else:
    ...
    if len(bubble_df) > 0:
        bubble_fig = px.scatter(
            bubble_df, x="n_agencies", y="base_count",
            size="equipment_value", color="region",
            color_discrete_map=REGION_COLORS,
            hover_name="state_name", template=PLOTLY_TEMPLATE,
            size_max=50,
            labels={"n_agencies": "Agencies Served", "base_count": "Number of Bases",
    ...
    return (dual_fig, scatter_fig, butterfly_fig, heatmap_fig, ranking_fig,
            temporal_fig, radar_fig, epb_fig, quadrant_fig, bubble_fig)
