...
ECG_SYNTH_PARAMS = {
    ...
    "Ventricular (V)": {"Q_amp": -0.15, "R_width": 0.025, "S_amp": -0.25,
                         "T_amp": 0.15, "T_center": 0.60, "T_width": 0.08},
    "Fusion (F)": {"R_width": 0.022, "S_amp": -0.20, "T_amp": 0.18, "Q_amp": -0.10},
    "Unknown (Q)": {"P_amp": 0.06, "R_amp": 0.65, "T_amp": 0.12, "R_width": 0.020},
    # PTB-DB classes
    "Normal": {},
    "Abnormal": {"Q_amp": -0.12, "S_amp": -0.22, "T_amp": 0.35,
                 "T_center": 0.50, "T_width": 0.055},
}
...
def detect_ecg_fiducials(waveform):
    ...
    result = {
        "R": None, "Q": None, "S": None, "P": None, "T": None,
        "PR_interval": None, "QT_interval": None, "ST_segment": None,
    }
...
def detect_multibeat_fiducials(full_waveform, beat_length):
    ...
    for i in range(n_beats):
        ...
        for key in ("P", "Q", "R", "S", "T"):
            pt = fid.get(key)
            if pt is not None:
                offset_fid[key] = {"index": pt["index"] + start,
...
def _build_fiducial_figure(waveform, fiducials):
    ...
    for key, fill_color, label in band_meta:
        ...
        if iv and iv.get("start") is not None:
            ...
            fig.add_annotation(
                x=(s + e) / 2, y=y_max + pad, text=label,
                showarrow=False, font=dict(size=9, color="#94A3B8"), yshift=6,
            )

    # Waveform trace
    fig.add_trace(go.Scatter(
        x=x_full, y=sig.tolist(), mode="lines",
        line=dict(color=COLORS["secondary"], width=1.8), name="ECG Signal",
    ...
    fig.update_layout(
        ...
        xaxis=dict(title="Sample Point"), yaxis=dict(title="Amplitude"),
        legend=dict(orientation="h", yanchor="bottom", y=1.02, xanchor="left", x=0),
        margin=dict(l=0, r=10, t=30, b=0), height=280,
    )
    return fig
...
def _build_fiducial_table(fiducials):
    ...
    rows = []
    for key, label in [("P", "P Wave Peak"), ("Q", "Q Point"), ("R", "R Peak"),
                        ("S", "S Point"), ("T", "T Wave Peak")]:
        pt = fiducials.get(key)
        if pt:
            rows.append(html.Tr([
    ...
    for key, label in [("PR_interval", "PR Interval"), ("QT_interval", "QT Interval"),
    ...
    header = html.Thead(html.Tr([
        html.Th("Feature"), html.Th("Index / Range"), html.Th("Amplitude / Length"),
    ]))
    return dbc.Table(
...
def layout():
    return html.Div([
                ...
                dbc.Card(dbc.CardBody([
                    html.H5(
                        [html.Span("ECG Playback Monitor"),
                         html.I(className="bi bi-info-circle ms-2 chart-info-icon", id="ecg-playback-info-icon")],
                        className="chart-title",
                        style={"display": "flex", "alignItems": "center"},
                    ),
                    dbc.Tooltip(
                        "Animated ECG strip chart with real-time PQRST fiducial detection. Synthetic waveforms are generated based on the selected heartbeat class. Use Play/Pause and Speed controls.",
                        target="ecg-playback-info-icon", placement="right",
                    ),
                    # Controls row
                    dbc.Row([
                        dbc.Col([
                            dbc.ButtonGroup([
                                dbc.Button("Play", id="ecg-play-btn", size="sm", color="success"),
                                dbc.Button("Reset", id="ecg-reset-btn", size="sm", outline=True, color="secondary"),
                            ]),
                        ], md=3),
                        dbc.Col([
                            html.Label("Speed", className="filter-label me-2",
                        ...
                        dbc.Col([
                            dbc.Badge("Stopped", id="ecg-playback-status",
                ...
                dbc.Card(dbc.CardBody([
                    html.H5(
                        [html.Span("Fiducial Points"),
                         html.I(className="bi bi-info-circle ms-2 chart-info-icon", id="ecg-fiducial-info-icon")],
                        className="chart-title",
                        style={"display": "flex", "alignItems": "center"},
                    ),
                    dbc.Tooltip(
                        "Annotated single-beat waveform showing detected P, Q, R, S, T points and derived intervals (PR, QT, ST). Shaded bands highlight interval regions.",
                        target="ecg-fiducial-info-icon", placement="right",
                    ),
        ...
        # Class distribution
        dbc.Row([
            dbc.Col(chart_container("ecg-class-dist", "Class Distribution (Train vs Test)", info="Grouped bar chart comparing heartbeat class distribution between training and test datasets. Use the scale toggle to switch between linear and log."), md=6),
            dbc.Col(chart_container("ecg-waveform-overlay", "Mean Waveform by Class (+/- 1 Std Dev)", info="Mean waveform overlay for each heartbeat class with shaded bands showing +/- 1 standard deviation of signal variability"), md=6),
        ]),
                ...
                dbc.Card(dbc.CardBody([
                    html.H5(
                        [html.Span("Waveform Browser"),
                         html.I(className="bi bi-info-circle ms-2 chart-info-icon", id="ecg-browser-info-icon")],
                        className="chart-title",
                        style={"display": "flex", "alignItems": "center"},
                    ),
                    dbc.Tooltip(
                        "Browse individual ECG samples one at a time. Select a class from the dropdown and navigate with Prev/Next buttons.",
                        target="ecg-browser-info-icon", placement="right",
                    ),
                    dbc.Row([
                        dbc.Col([
                        ...
                        dbc.Col([
                            ...
                            dbc.ButtonGroup([
                                dbc.Button("< Prev", id="ecg-prev-btn", size="sm", outline=True, color="primary"),
                                html.Span(id="ecg-sample-idx", className="mx-3 align-self-center", style={"fontWeight": "600"}),
                                dbc.Button("Next >", id="ecg-next-btn", size="sm", outline=True, color="primary"),
                            ]),
                        ], md=4),
                    ], className="mb-2"),
                    dcc.Loading(dcc.Graph(id="ecg-waveform-browser", style={"height": "350px"}), type="circle"),
                ]), className="chart-card"),
            ], md=12),
        ]),
        ...
        # Features + Heatmap
        dbc.Row([
            dbc.Col(chart_container("ecg-features", "Signal Feature Comparison", info="Grouped bar chart comparing extracted signal features (peak amplitude, energy, zero crossings) across heartbeat classes"), md=6),
            dbc.Col(chart_container("ecg-correlation", "Class Similarity Heatmap", info="Correlation matrix showing pairwise similarity between heartbeat classes based on mean waveform shape. Values range from -1 to 1."), md=6),
        ]),

        # PCA Embedding
        dbc.Row([
            dbc.Col(chart_container("ecg-pca-scatter", "PCA Class Embedding", info="2D PCA projection of sampled ECG waveforms, colored by class label. Shows class separability in reduced feature space. Explained variance shown in axis labels."), md=12),
        ]),
...
def update_class_distribution(dataset, scale):
    ...
    for split_name in ["train", "test"]:
        if split_name in ds["splits"]:
            ...
            fig.add_trace(go.Bar(
                y=list(dist.keys()), x=list(dist.values()),
                name=split_name.capitalize(), orientation="h",
            ))
    ...
    fig.update_layout(
        ...
        yaxis=dict(title=""),
        legend=dict(orientation="h", yanchor="bottom", y=1.02, xanchor="left", x=0),
        margin=dict(l=0, r=10, t=10, b=0), height=400,
    )
    return fig
...
def update_waveform_overlay(dataset):
    ...
    for i, class_name in enumerate(split["mean_waveforms"]):
        ...
        # Mean line
        fig.add_trace(go.Scatter(
            x=x_axis, y=mean.tolist(),
            name=class_name, line=dict(color=color, width=2),
        ))
...
def update_fiducial_panel(waveform):
    ...
    if not waveform or len(waveform) < 10:
        ...
        empty.update_layout(
            ...
            annotations=[dict(text="No waveform selected", showarrow=False,
...
def build_playback_figure(waveform):
    ...
    fig.add_trace(go.Scatter(
        x=[], y=[], mode="lines",
        line=dict(color=COLORS["danger"], width=2), name="ECG Trace",
    ))
    ...
    if waveform and len(waveform) > 1:
        ...
        fig.update_layout(
            ...
            yaxis=dict(title="Amplitude (mV)",
                       range=[float(sig.min()) - 0.1, float(sig.max()) + 0.15],
                       autorange=False),
        ...
        fig.update_layout(
            template=PLOTLY_TEMPLATE, height=550,
            margin=dict(l=0, r=10, t=10, b=0),
            annotations=[dict(text="Select a class and press Play",
...
def update_pca_scatter(dataset):
    ...
    if "pca_embedding" not in ds:
        ...
        fig.update_layout(
            ...
            annotations=[dict(text="PCA data not available — regenerate cache", showarrow=False,
                              xref="paper", yref="paper", x=0.5, y=0.5,
                              font=dict(size=14, color="#999"))],
        )
        return fig
