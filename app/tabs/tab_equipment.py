...
def layout():
    ...
    return html.Div([
        ...
        dbc.Row([
            dbc.Col(chart_container("equip-timeline", "Transfers Over Time", height=450, info="Animated bar chart tracking cumulative transfer quantities over time for the top 5 equipment categories. Press play to watch transfers accumulate year by year."), md=7),
            dbc.Col([
                chart_container("equip-top-items", "Top 15 Equipment Types by Quantity", info="Horizontal bar chart ranking the 15 most frequently transferred equipment types by total quantity"),
            ], md=5),
        ]),
                ...
                dbc.Card(dbc.CardBody([
                    html.H5(
                        [html.Span("Equipment Transfers by State"),
                         html.I(className="bi bi-info-circle ms-2 chart-info-icon", id="equip-choropleth-info-icon")],
                        className="chart-title",
                        style={"display": "flex", "alignItems": "center"},
                    ),
                    dbc.Tooltip(
                        "Choropleth map showing geographic distribution of military equipment transfers. Toggle between total acquisition value and item count using the radio buttons above.",
                        target="equip-choropleth-info-icon", placement="right",
                    ),
                    dbc.RadioItems(
                        id="equip-map-metric",
                        options=[
                    ...
                    dcc.Loading(dcc.Graph(id="equip-choropleth", style={"height": "450px"}), type="circle"),
                ]), className="chart-card"),
            ], md=7),
            dbc.Col(chart_container("equip-yoy", "Year-over-Year Transfer Growth Rate", info="Bar chart showing annual percentage change in total acquisition value. Green bars indicate growth, red bars indicate decline."), md=5),
        ]),

        # Per-Capita Choropleth
        dbc.Row([
            dbc.Col(chart_container("equip-percapita", "Per-Capita Equipment Value by State", info="Choropleth map showing equipment acquisition value normalized by state population (2020 Census). Reveals disproportionate transfers to smaller states."), md=12),
        ]),

        # --- Equipment Type Breakdowns ---
        dbc.Row([
            dbc.Col(chart_container("equip-demil", "DEMIL Code Breakdown", info="Stacked bar chart showing equipment sensitivity/restriction levels (DEMIL codes) across top categories by acquisition value"), md=6),
            dbc.Col(chart_container("equip-top-states", "Top 10 States by Acquisition Value", info="Horizontal bar chart showing the 10 states with the highest total equipment acquisition values, colored by census region"), md=6),
        ]),

        # Treemap
        dbc.Row([
            dbc.Col(chart_container("equip-treemap", "Acquisition Value by Category", info="Treemap showing relative acquisition value of equipment grouped by category and item name. Larger rectangles indicate higher value."), md=12),
        ]),

        # --- Rankings & Derived Metrics ---
        dbc.Row([
            dbc.Col(chart_container("equip-diversity", "Average Value per Item by State", info="Horizontal bar chart showing average acquisition value per transferred item for the top 25 states — reveals which states receive more expensive equipment"), md=6),
            dbc.Col(chart_container("equip-top-agencies", "Top 20 Agencies by Acquisition Value", info="Horizontal bar chart showing the 20 agencies with the highest total equipment acquisition values"), md=6),
        ]),
...
def update_equipment(states, year_range, categories, map_metric):
    ...
    kpis = dbc.Row([
        dbc.Col(kpi_card("Total Items", total_items, color=COLORS["secondary"], info="Total quantity of all equipment items transferred under the 1033 Program"), md=3),
        dbc.Col(kpi_card("Total Value", total_value, color=COLORS["success"], info="Combined acquisition value of all transferred military equipment"), md=3),
        dbc.Col(kpi_card("Agencies Served", n_agencies, color=COLORS["warning"], info="Number of unique law enforcement agencies that received equipment"), md=3),
        dbc.Col(kpi_card("States & Territories", n_states, color=COLORS["info"], info="Number of US states and territories with recorded equipment transfers"), md=3),
    ])
    ...
    choropleth = px.choropleth(
        ...
        color=color_col, color_continuous_scale="Blues",
        scope="usa", template=PLOTLY_TEMPLATE,
    ...
    choropleth.update_layout(
        ...
        geo=dict(
            ...
            showlakes=True, lakecolor="#0F1726",
            showcountries=True, countrycolor="rgba(99,125,175,0.2)",
            showsubunits=True, subunitcolor="rgba(99,125,175,0.15)",
        ),
    )
    ...
    top_items_fig = px.bar(
        top_items, x="Quantity", y="Item Name", orientation="h",
    ...
    if len(timeline_df) > 0:
        ...
        for yr in all_years:
            ...
            for cat in top_cats:
                ...
                frame_traces.append(go.Scatter(
                    x=cat_df["Year"], y=cat_df["Quantity"],
        ...
        timeline_fig.update_layout(
            ...
            updatemenus=[dict(
                ...
                bgcolor="#1A2332", font=dict(color="#E2E8F0"),
                buttons=[
            ...
            sliders=[dict(
                active=0, steps=[
                    dict(args=[[str(yr)], {"frame": {"duration": 400, "redraw": True}, "mode": "immediate"}],
                         method="animate", label=str(yr))
                    for yr in all_years
                ],
                x=0.15, len=0.85, y=-0.12,
                currentvalue=dict(prefix="Year: ", font=dict(color="#E2E8F0")),
                font=dict(color="#E2E8F0"),
                bgcolor="#1A2332", bordercolor="rgba(99,125,175,0.3)",
            )],
        )
    else:
        timeline_fig = go.Figure()
    ...
    # Treemap
    treemap_df = df.groupby(["Category", "Item Name"]).agg(
        Value=("Acquisition Value", "sum"), Count=("Quantity", "sum")
    ).reset_index()
    treemap_df = treemap_df[treemap_df["Value"] > 0]
    if len(treemap_df) > 0:
        treemap_df = treemap_df.sort_values("Value", ascending=False).groupby("Category").head(10).reset_index(drop=True)
        treemap_fig = px.treemap(
            treemap_df, path=["Category", "Item Name"], values="Value",
            template=PLOTLY_TEMPLATE, color="Value",
    ...
    agencies_fig = px.bar(
        top_agencies, x="Acquisition Value", y="Agency Name", orientation="h",
    ...
    percapita_fig = px.choropleth(
        state_value, locations="State", locationmode="USA-states",
        color="Per Capita Value", color_continuous_scale="Reds",
        scope="usa", template=PLOTLY_TEMPLATE,
    ...
    demil_agg = demil_df[demil_df["Category"].isin(top_demil_cats)].groupby(
        ["Category", "DEMIL Label"]
    )["Acquisition Value"].sum().reset_index()

    if len(demil_agg) > 0:
        demil_fig = px.bar(
            demil_agg, x="Acquisition Value", y="Category", color="DEMIL Label",
    ...
    top_states_fig = px.bar(
        top_states_df, x="Value", y="State", orientation="h",
        template=PLOTLY_TEMPLATE, color="Region", color_discrete_map=REGION_COLORS,
    ...
    diversity_fig = px.bar(
        state_avg, x="Avg Value per Item", y="State", orientation="h",
        template=PLOTLY_TEMPLATE, color_discrete_sequence=[COLORS["accent"]],
    ...
    if len(yearly_val) > 1:
        ...
        yoy_fig = go.Figure(go.Bar(
            x=yoy_df["Year"], y=yoy_df["Growth Rate (%)"],
    ...
    return (kpis, choropleth, top_items_fig, timeline_fig, treemap_fig, agencies_fig,
            percapita_fig, demil_fig, top_states_fig, diversity_fig, yoy_fig)
